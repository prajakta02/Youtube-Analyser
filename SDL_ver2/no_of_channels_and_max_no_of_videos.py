...
Label(root, text='~ No of channels in data base ~',font=(" ",20),bg='black',fg='white').place(x=350,y=140)
Label(root, text=channel_len,font=(" ",15),width=50).place(x=350,y=200)



max_no_of_videos = max(youtube_vid['channel_title'].value_counts(ascending=True))
Label(root, text='~ Max no of videos by a single channel ~',font=(" ",20),bg='black',fg='white').place(x=350,y=350)
Label(root, text=max_no_of_videos,font=(" ",15),width=50).place(x=350,y=410)
...
exit = Button(root, text = ' Exit ', fg ='white',width=12,height=1, bd = '0',activeforeground='red',bg='black',command=root.destroy,font=(" ",30)).place(x=610,y=510)
root.mainloop()